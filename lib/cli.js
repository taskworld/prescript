...
function runDevelopmentMode (testModulePath) {
  ...
  function announceStatus () {
    ...
    if (current) {
      console.log(chalk.bold.blue('次は ') + prettyFormatStep(tester.getCurrentStep()))
...
function createLogVisitor () {
  return {
    visitNode (node) {
      if (node.children) {
        console.log(chalk.dim('入  '), prettyFormatStep(node))
      }
    }
  }
}
...
function * runNext (tester, state, onError) {
  ...
  process.stdout.write(chalk.dim('Runs ') + prettyFormatStep(step))
  const started = Date.now()
  const formatTimeTaken = () => chalk.dim(ms(Date.now() - started))
  try {
    yield step.action(state)
    console.log(chalk.bold.green(' OK'), formatTimeTaken())
    tester.actionPassed()
  } catch (e) {
    console.log(chalk.bold.red(' ERROR'), formatTimeTaken())
    console.log(chalk.red(e.stack))
    onError(e)
    tester.actionFailed()
  }
