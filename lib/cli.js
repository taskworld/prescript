...
function runDevelopmentMode (testModulePath) {
  ...
  .action(function (args, callback) {
    handleRun(co(function * () {
      yield runTestWhileConditionMet()
    }), callback)
  })
  ...
  const runTestWhileConditionMet = function * (condition) {
  ...
  .action(function (args, callback) {
    handleRun(co(function * () {
      if (!isStepExist(tester.getTest(), String(args.stepNumber))) {
...
function runNonInteractiveMode (testModulePath) {
  ...
  function createReporter () {
    ...
    allure.startSuite('prescript')
    allure.startCase(testModulePath)
    return {
      iterationListener: {
        onEnter (node) {
          allure.startStep(node.name, Date.now())
        },
        onExit (error) {
      ...
      onFinish (errors) {
        allure.endCase(errors.length ? 'failed' : 'passed', errors[0])
        allure.endSuite()
      }
    }
  }
